...
# the letter was correct or incorrect. If out of guesses, the player should lose. Now
# implement the functionality where, at the start of any turn, instead of making a
...
# keep playing until game_over? is true
  def play
    puts random_word
    turn_number = 0
  ...
  def win?
    if @guess_word.join('').eql?(@hangman_word)
      true
    end
  end
  ...
  def each_turn(turn_number)
    display_secret_word_length
    prompt_for_letter
    display_guessed_letters
    join_guess_word_into_string_if_there_is_a_match
    display_turns_left(turn_number)
  end

  def join_guess_word_into_string_if_there_is_a_match
    # this puts is only here for testing purposes, will be removed
    puts "hangman word is: #{@hangman_word}"
    if @hangman_word.include?(@prompt_guess_letter)
      @guess_word << @prompt_guess_letter
      puts guess_word_confirmation(@guess_word)
      # display_secret_word_length.replace('-', @prompt_guess_letter)
    else
...
#    @guess_word
  ...
  def display_secret_word_length
    @hangman_word.length.times { puts '_' }
  end
